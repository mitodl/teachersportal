...
from teachersportal.settings import load_fallback, get_var
...
class TestSettings(TestCase):
    ...
    def reload_settings(self):
        ...
        """
        imp.reload(sys.modules['teachersportal.settings'])
        # Restore settings to original settings after test
        self.addCleanup(imp.reload, sys.modules['teachersportal.settings'])
        return vars(sys.modules['teachersportal.settings'])
    ...
    def test_get_var(self):
        ...
        with mock.patch.dict(
            'teachersportal.settings.FALLBACK_CONFIG',
            {'FOO': 'bar'}
        ):
            # Verify fallback
    ...
    def test_admin_settings(self):
        ...
        with mock.patch.dict('os.environ', {
            'PORTAL_ADMIN_EMAIL': test_admin_email,
        }, clear=True):
            settings_vars = self.reload_settings()
            self.assertEqual(
